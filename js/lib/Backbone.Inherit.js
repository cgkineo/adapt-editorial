;define(['backbone'],function(Backbone){
	...
	Inherit.prototype = {
		...
		callParents: function(name, callback, defer) {
			...
			var binder = function() {
				...
				for (var i = 0, l = this._inheritParents.length; i < l; i++) {
					...
					//check if the parent function is already callParents bound
					if (callbackFunctionAtParent._performsInherited) {
						...
						//the parent function is not bound by callParents so it can be called directly
	...
	Inherit.callParents = Inherit.prototype.callParents;
	Inherit.extend = function(to) {
		return _.extend({}, Inherit.prototype, to);
	};

	//extend Backbone with the two functions callParents and getInheritParents
	_.extend(Backbone, Inherit.prototype);

	/* result
	* Backbone.callParents("functionName", function(sharedArgument) {
	*
	* });
